...
def main():
    ...
    # Calculate total prices and bill
    appetizers_price = (chilli_baby_corn * 12 + chilli_paneer * 14 + chill_gobi * 13 +
                        gobi_manchurian * 12 + paneer_manchurian * 14 + paneer_65 * 14 +
                        paneer_kathi_roll * 10)
    main_dishes_price = (plain_ghee_dosa * 10 + podi_dosa * 13 + masala_dosa * 13 +
                         podi_idli * 9 + idli_sambar * 9 + pongal * 10 + aloo_puri * 13)
    chaat_pakoda_price = (samosa_chaat * 9 + papri_chaat * 9 + vada_pav * 8 +
                          pani_puri * 8 + sev_puri * 9 + aloo_tiki * 9 + mirchi_chaat * 9)

    total_price = appetizers_price + main_dishes_price + chaat_pakoda_price
    tax = total_price * 0.1
    service_charge = 5
    total_bill = total_price + tax + service_charge

    # Display the billing summary below the title bar
    st.markdown(
        """
        <div class="billing-box">
            <div class="billing-summary">Bill Summary</div>
            <div class="billing-item">Bill Number: {}</div>
            <div class="billing-item">Customer Name: {}</div>
            <div class="billing-item">Phone Number: {}</div>
            <div class="billing-item">Total Appetizers Price: ${}</div>
            <div class="billing-item">Total Main Dishes Price: ${}</div>
            <div class="billing-item">Total Chaat & Pakoda Price: ${}</div>
            <div class="billing-item">Total Tax: ${}</div>
            <div class="billing-item">Service Charge: ${}</div>
            <div class="billing-item">Total Bill: ${}</div>
        </div>
        """.format(
            bill_no, c_name, phone,
            round(appetizers_price, 2),
            round(main_dishes_price, 2),
            round(chaat_pakoda_price, 2),
            round(tax, 2),
    ...
    if user_input:
        ...
        st.sidebar.text_area("Ask something...", value=response, height=150, max_chars=None, key=None)
